...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
			...
			<p class="text-muted mb-0">Find your next pre-loved read.</p>
		</div>
    </div>
    ...
    <form method="get" class="card p-3 mb-4">
        <div class="row g-3">
            <div class="col-md-5">
				<input name="search" class="form-control" value="<?= htmlspecialchars($search) ?>" placeholder="Search title or author">
			</div>
            <div class="col-md-3">
				<select name="genre" class="form-select">
					<option value="">All genres</option>
					<?php while ($item = mysqli_fetch_assoc($genres)): ?>
					<option value="<?= htmlspecialchars($item['genre']) ?>" <?= $genre === $item['genre'] ? 'selected' : '' ?>><?= htmlspecialchars($item['genre']) ?></option>
					<?php endwhile; ?>
				</select>
			</div>
            <div class="col-md-3">
				<select name="language" class="form-select">
					...
					<option value="<?= htmlspecialchars($item['language']) ?>" <?= $language === $item['language'] ? 'selected' : '' ?>><?= htmlspecialchars($item['language']) ?></option>
					<?php endwhile; ?>
				</select>
			</div>
