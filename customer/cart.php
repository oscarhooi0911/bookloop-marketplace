    ...
    if ($action === 'add') {
        ...
        if (mysqli_stmt_get_result($book)->fetch_assoc()) {
            $add = mysqli_prepare($conn, 
									...
									VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE quantity = LEAST(quantity + VALUES(quantity), 10)');
...
$stmt = mysqli_prepare($conn, 
						'SELECT c.cart_id, c.quantity, b.title, b.price 
						FROM cart c 
...
<div class="container py-5">
	<div class="d-flex justify-content-between align-items-center mb-4">
		...
		<a class="btn btn-outline-primary" href="browse_books.php">Continue browsing</a>
	</div>
	<?php if (mysqli_num_rows($items) === 0): ?>
	<div class="alert alert-info">Your cart is empty.</div>
		<?php else: $total = 0; ?><div class="table-responsive">
		<table class="table align-middle">
			...
			<tbody><?php while ($item = mysqli_fetch_assoc($items)): $subtotal = $item['price'] * $item['quantity']; $total += $subtotal; ?>
				<tr>
					<td><?= htmlspecialchars($item['title']) ?></td><td>$<?= number_format((float) $item['price'], 2) ?></td>
